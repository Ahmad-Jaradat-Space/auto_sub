...
- `ffmpeg.exe` / `ffprobe.exe` are fetched by CI from gyan.dev's *essentials* build (has libass) into `packaging/ffmpeg/` and land in an `ffmpeg/` folder next to the exe. `core/burn.py::_bundle_dirs()` looks there first, before the Homebrew paths, before `PATH`. The workflow asserts the `ass` filter exists rather than shipping a build that cannot burn.
